...
def sos_lasso(params, X, y, alpha):
    SSE = sos(params, X, y)  # Sum of squares of residuals
    penalty = np.sum(params[:-1])
    n = len(y)
    return 1 / (2 * n) * SSE + alpha * penalty
```
...
```{python}
lm1_fit = sklm.LinearRegression().fit(
    clarity_df, helpfulness)
predictions = lm1_fit.predict(clarity_df)
np.sum((helpfulness - predictions) ** 2)
```

```{python}
lm2_fit = sklm.LinearRegression().fit(
    clarity_easiness, helpfulness)
predictions2 = lm2_fit.predict(clarity_easiness)
np.sum((helpfulness - predictions2) ** 2)
```

Oops - the model with `Easiness` has actually got a slightly higher absolute error than the one without.  We should probably prefer the one without.

```{python}
def drop_and_predict(df, x_cols, y_col, to_drop):
    """ Drop value identified by `to_drop`, fit with rest and predict
    """
...
predicted_value = drop_and_predict(df, ['Clarity', 'Easiness'], ['Helpfulness'], 0)
